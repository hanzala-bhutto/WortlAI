...
```
┌────────────────────────  React SPA, Vite dev on localhost:3001 ──────────────────────────────┐
...
| Observability | Langfuse, self-hosted on the local v3 stack, via a WortlAI-specific Langfuse project | Traces, prompt versioning, evals |

API keys needed (all free): Groq, NVIDIA NIM (build.nvidia.com). Langfuse keys come from the **local self-hosted** instance, not a cloud account. Qdrant runs locally via docker compose.
...
│   └── requirements.txt
├── frontend/                       # React SPA (Vite); builds to dist/, served by FastAPI
│   └── src/                        # routes/ (talk, review, dashboard), components/,
│                                   # lib/api.ts, audio hooks
...
1. Scaffold repo: FastAPI backend + React/Vite frontend + docker-compose (Qdrant, unused yet).
...
- **/dev** - start full stack (docker compose: Qdrant; uvicorn; vite dev), health-check all services.
...
- Stack: React SPA (Vite) + FastAPI, served from one origin in production. TTS: Edge TTS (Whisper is STT-only - it cannot speak). STT/LLM: Groq free tier, NIM fallback. Local-only deployment, single user, no auth. Build order: Phase 1 voice loop first.
